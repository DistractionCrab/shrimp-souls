...
class ArenaBase(persistent.Persistent):
	def __init__(self):		
	...
	def join(self, player):
		...
		if player.name in PLAYERS:
			...
			print(f"{player.name} has joined the arena! They have been healed and refreshed. Wokege")
...
class Setup(ArenaBase):
	...
	def enter(self, previous):
		...
		import ShrimpSouls.campaigns as cps
		if not isinstance(previous, Arena):
			PLAYERS.clear()
...
class Arena(ArenaBase):
	...
	def __npcs(self):
		if self.__npc_list is None:
			self.__npc_list = list(ENEMIES[p] for p in ENEMIES)
	...
	def perform_action(self, pname):
		...
		if pname.lower() not in PLAYERS:
			...
			if p.acted:
				...
				print(f"{p.name} cannot perform actions while dead. Deadge")
	...
	def use_ability(self, name, abi, targets):
		if pname.lower() not in PLAYERS:
			...
			if p.acted:				
				...
				print(f"{p.name} cannot perform actions while dead. Deadge")
				return f"{p.name} cannot perform actions while dead. Deadge"
			elif p.status.stun > 0:
				print(f"{p.name} cannot perform actions while stunned.")
				return f"{p.name} cannot perform actions while stunned."
			else:
